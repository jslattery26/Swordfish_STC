    ...
    public class Launcher : MonoBehaviourPunCallbacks
    {
        ...
        public void Connect()
        {
            ...
            if (PhotonNetwork.IsConnected)
            {
                ...
                // #Critical we need at this point to attempt joining a Room. If it fails, we'll get notified in OnJoinRoomFailed() and we'll create one.
...
//        }
        ...
        {
            Debug.Log("PUN Basics Tutorial/Launcher: OnJoinedRoom() called by PUN. Now this client is in " + roomName);

            // #Critical: We only load if we are the first player, else we rely on `PhotonNetwork.AutomaticallySyncScene` to sync our instance scene.
            if (PhotonNetwork.CurrentRoom.PlayerCount <= 4)
            {
                Debug.Log("We load the" + roomName);
